...
class PumpRepositoryImpl implements PumpRepository {
  ...
  List<PumpModel> _filterAndSortByRadius(
    ...
    debugPrint(
      '[PumpRepositoryImpl] user location: '
    ...
    for (final entry in withDistance) {
      final kept = entry.distanceKm <= radiusKm;
      debugPrint(
        '[PumpRepositoryImpl] pump="${entry.pump.name}" '
        'distanceKm=${entry.distanceKm} '
        '${kept ? 'KEPT' : 'REMOVED'}',
      );
    }

    final withinRadius = withDistance
        .where((entry) => entry.distanceKm <= radiusKm)
        .toList()
      ..sort((a, b) => a.distanceKm.compareTo(b.distanceKm));

    return withinRadius.map((entry) => entry.pump).toList();
  }
  ...
  Stream<List<PumpModel>> watchAllPumps() {
    return _remoteDataSource.watchAllPumps().handleError((Object e) {
      throw ErrorHandler.handle(e, context: 'watchAllPumps');
    });
  }
